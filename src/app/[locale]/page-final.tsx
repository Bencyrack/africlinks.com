...
import { ArrowRight, Calendar, Users, Globe, TrendingUp, Award, BookOpen, ChevronLeft, ChevronRight } from 'lucide-react';

export default function HomePage() {
  // Carrousel d'images pour le Hero
  const heroImages = [
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center text-white relative z-20">
          ...
          <p className="text-xl md:text-2xl mb-8 max-w-3xl mx-auto animate-fade-in-up animation-delay-200">
            Promouvoir l'économie, la diversité culturelle, l'éducation et la coopération entre les pays africains francophones
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {news.map((item) => (
              <article key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
                <div className="relative h-48 bg-linear-to-br from-purple-400 to-yellow-400 overflow-hidden">
                  <Image
                    src={item.image}
                    alt={item.title}
                    fill
